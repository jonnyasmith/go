...
Capacity is divided evenly across shards and must provide at least the fixed entry-representation charge per shard. That charge is derived from the Go `entry` structure and is 72 bytes on supported 64-bit targets; each entry is charged for that representation, its key, and its value. `WithCapacity` is therefore a ceiling on these charged bytes, not on process RSS. Allocator metadata, map buckets, goroutine stacks, temporary I/O buffers, and Go runtime overhead are outside the charge. The sweep visits shards at staggered offsets, samples entries within each shard, repeats samples while at least one quarter are reclaimed, and spends at most one millisecond in a shard per visit.
...
Eviction never becomes a durable deletion. After any live-entry eviction, each automatic compaction rolls the writer to a new segment, reconstructs the durable image from the installed snapshot and immutable WAL prefix, and snapshots that image before pruning only the covered prefix. The configured snapshot threshold remains the compaction trigger under sustained pressure: retained WAL is bounded by that threshold plus records accepted while one compaction runs. Clean shutdown reconstructs an eviction-aware final image one configured shard at a time, so it does not allocate a second full durable store. Its additional live-entry memory is bounded by the largest reconstructed durable shard, plus map and decoding overhead; its tradeoff is replaying the installed snapshot and retained WAL once per configured shard, for $O(\text{shards}\times(\text{snapshot bytes}+\text{WAL bytes}))$ reconstruction work before writing the final image. [`BenchmarkFinalSnapshotAfterEviction`](operational_bounds_test.go) records allocations for this close path. Reopening with greater capacity restores live entries that had been evicted from memory.

## Observability

`Stats` returns a plain struct read from atomic counters — hits, misses, expiries, evictions, records and bytes written, fsyncs, snapshots, and the last error. `Snapshots` increments only after rename, directory synchronization, superseded-snapshot removal, and covered-segment removal all complete. An error after rename says that the snapshot is already installed while leaving the counter unchanged. A later successful automatic snapshot clears an earlier automatic-snapshot error; a fatal WAL error remains latched until reopen and takes precedence in `LastError`. Export these values however your program already exports things. The module deliberately offers no metrics interface and imports no metrics library: an indirect call on every cache hit would cost more than the counter feeding it.
