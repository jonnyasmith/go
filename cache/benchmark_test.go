...
func benchmarkStoreContention(b *testing.B, readPercent uint64) {
	for _, allocating := range []bool{true, false} {
		readName := "GetIntoReusable"
		if allocating {
			readName = "GetAllocating"
		}
		for _, skewed := range []bool{false, true} {
			distribution := "uniform-shards"
			if skewed {
				distribution = "skewed-hot-shard"
			}
			for _, goroutines := range []int{1, 2, 4, 8, 16} {
				b.Run(fmt.Sprintf("%s/%s/goroutines=%d", readName, distribution, goroutines), func(b *testing.B) {
