...
func TestSequenceFilenamesCoverUint64AndLegacyNamesRemainReadable(t *testing.T) {
	tests := []struct {
		...
		{math.MaxUint64, "18446744073709551615.seg", "18446744073709551615.snap"},
	}
...
func writeTestSnapshotHeader(t *testing.T, dir string, sequences []uint64) {
	...
	header := make([]byte, snapshotFixedHeaderSize+int64(len(sequences))*8)
	copy(header[:4], snapshotMagic[:])
	binary.LittleEndian.PutUint16(header[4:6], formatVersion)
	binary.LittleEndian.PutUint32(header[8:12], uint32(len(sequences)))
	for index, sequence := range sequences {
		binary.LittleEndian.PutUint64(header[snapshotFixedHeaderSize+int64(index)*8:], sequence)
	}
	if err := os.WriteFile(filepath.Join(dir, snapshotName(lowest)), header, 0o600); err != nil {
